...
n=1
echo "" > candidate_nodes_for_update_inventory
...
do
  ...
  echo $NODE_INVENTORY >> candidate_nodes_for_update_inventory
  n=$n+1
done

# Need to first copy status check script template.
cd ~/architecture2/monitoring-bag
cp roles/client/templates/check-seqware-sanger-last-workflow-status.json.j2 ./check-seqware-sanger-last-workflow-status.sh
...
# Tell ansible to run the script for all machines in nodes_for_update_inventory.
NODES_TO_UPDATE=()
...
do
  ...
  echo $HOST_LINE
  ANSIBLE_RESULT=$(ansible -v -i ~/architecture-setup/candidate_nodes_for_update_inventory host_$i -m script -a check-seqware-sanger-last-workflow-status.sh)
  ANSIBLE_STD_OUT=$(echo $ANSIBLE_RESULT |  sed 's/.*"stdout": "\([^"]*\)".*/\1/g')
  ANSIBLE_STD_ERR=$(echo $ANSIBLE_RESULT |  sed 's/.*"stderr": "\([^"]*\)".*/\1/g')
  if [[ $ANSIBLE_OUT =~ .*completed.* ]]
  then 
...
if [ ${#NODES_TO_UPDATE[@]} > 0 ] ; then
  echo "[master]" > ~/architecture2/pancancer-bag/nodes_to_update_inventory
  for node_line in "${NODES_TO_UPDATE[@]}"
  do  
    echo $node_line >> ~/architecture2/pancancer-bag/nodes_to_update_inventory
  done
fi
